	...
	def compile_sound(self):
		for i, cf in enumerate(self.codeFiles):
			...
			for j, f in enumerate(folder):
				...
				#教師データを定義
				y_data = np.full(Chromas.shape[0], i)
				...
				np.savetxt(fname=x_output, X=Chromas, fmt="%.18e")
				np.savetxt(fname=y_output, X=y_data, fmt="%s")

				del Chromas
				del y_data
	
	def get_chromas(self, train_range=[1,2,3,4,5,6,7,8], test_range=[9,10]):
		print("train_range=" + str(train_range))
		print("test_range=" + str(test_range))
		x_train = np.zeros(12)
		x_test = np.zeros(12)
		y_train = np.empty(1)
		y_test = np.empty(1)
